...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        if self.get_dirt_amount(tile_x,tile_y)<capacity:
    ...
    def is_position_in_room(self, pos):
        ...
        """
        if (math.floor(pos.get_x()),math.floor(pos.get_y())) in self.tiles.keys():
            return True
        return False
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        self.posi=room.get_random_position()
        self.direction=random.random()*360
        #print(self.direction,"class Robot")
    ...
    def set_robot_direction(self, direction):
        ...
        if 0.0<=direction<360:
            ...
            #print(self.direction,"set_robot_dir")
        else:
            raise ValueError
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        """
        return self.is_position_in_room(pos) and not(self.is_tile_furnished(math.floor(pos.get_x()),math.floor(pos.get_y())))
    ...
    def get_random_position(self):
        ...
        ran_y=random.random()*self.height

        while self.is_position_valid(Position(ran_x,ran_y)):
            ran_x = random.random() * self.width
            ran_y = random.random() * self.height
        return Position(ran_x,ran_y)
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        curr_dir=self.get_robot_direction()
        curr_speed=self.speed
        new_pos=curr_pos.get_new_position(curr_dir,curr_speed)
        ...
        if self.room.is_position_valid(new_pos):
            ...
            self.direction =self.get_robot_direction()
            #print(self.direction,"update and clean")
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        if not self.gets_faulty():
            new_pos=self.posi.get_new_position(self.direction,self.speed)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        anim = ps3_visualize.RobotVisualization(num_robots, width, height, False, 0.00001)
        room = EmptyRoom(width, height, dirt_amount)
        curr_cov=0
        time=0
        robots=[]
        for ro in range(num_robots):
                robot = robot_type(room, speed, capacity)
                posi=room.get_random_position()
                robot.set_robot_position(posi)
                robots.append(robot)
        while curr_cov<=min_coverage:
            time+=1
            for rob in robots:
                rob.update_position_and_clean()
                anim.update(room, robots)
                curr_cov=room.get_num_cleaned_tiles()/room.get_num_tiles()
        anim.done()
