...
class WeightedEdge(Edge):
    ...
    def __str__(self):
        #statement= self.src.get_name() + '->' + self.dest.get_name() + ' (' + str(self.get_total_distance()) + ', ' + \
        str(self.get_outdoor_distance()) + ')'
        print('gdsfg')
        statement = self.src.get_name()+"->"+self.dest.get_name()+" ("+str(self.get_total_distance())+", "+str(self.get_outdoor_distance())+")"
        return statement
...
class Digraph(object):
    ...
    def add_edge(self, edge):
        ...
        in the  graph."""
        fin=WeightedEdge(edge.get_source(),edge.get_destination(),edge.total_distance,edge.outdoor_distance)
        if not (edge.get_source()  in self.nodes and edge.dest in self.nodes):
            raise ValueError
        self.edges[edge.get_source()].append(fin)
