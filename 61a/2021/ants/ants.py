...
class Ant(Insect):
    ...
    def add_to(self, place):
       # print(f"self={self},self.can_contain={self.can_contain(place.ant)}")
        if place.ant!=None and place.ant.can_contain(self):
            place.ant.ant_contained=self
        elif place.ant!=None and self.can_contain(place.ant):
            x=place.ant
            place.ant=self
            place.ant.ant_contained=x
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        pla = self.place
        dist = 0
        while not pla.is_hive:
            print(f"pla={pla}")
            print(f"self.min={self.min_range},self.max={self.max_range},dist={dist}")
            print(f"{self.min_range<=dist<=self.max_range}")
            if self.min_range <= dist and dist <= self.max_range:
                bee = choose_bee(pla.bees)
                print(f"bee={bee}")
                if bee != None:
                    return bee
                else:
...
class HungryAnt(Ant):
    ...
    def action(self,gamestate):
        if self.chew_countdown!=0:
            ...
            dead_bee=choose_bee(self.place.bees)
            if dead_bee!=None:
                dead_bee.reduce_health(dead_bee.health)
